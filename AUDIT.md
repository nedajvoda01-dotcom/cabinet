...
- **P1 – Source-of-truth Spec missing** — Autocontent `Spec.htm` is absent from the repo, leaving behavior without the declared source of truth. Add the spec file to the root so implementations/tests can align to it.【F:AGENTS.md†L250-L255】【c72b74†L1-L2】
...
## Quick wins (≤1h)
- Add stub `docs/`, `infra/`, and `.github/workflows` nodes with placeholder readmes to align with the frozen tree and unblock documentation workstreams.【F:AGENTS.md†L39-L150】
- Introduce a repo-local `Autocontent Spec.htm` placeholder (or link) to satisfy the declared source of truth and reference it from module docs/tests.【F:AGENTS.md†L250-L255】
- Replace TODOs in `ParserJobs`, `ExportJobs`, `PublishJobs`, and `PhotosJobs` with minimal queue dispatch hooks to enable end-to-end task submission without changing business logic.【F:backend/src/Modules/Parser/ParserJobs.php†L12-L35】【F:backend/src/Modules/Export/ExportJobs.php†L12-L34】【F:backend/src/Modules/Publish/PublishJobs.php†L12-L43】【F:backend/src/Modules/Photos/PhotosJobs.php†L12-L35】
